...
#else
#define FMADD(a, b, c)		_mm_macc_ps(a, b, c)
#define FMADD256(a, b, c)	_mm256_add_ps(c, _mm256_mul_ps(a, b)); // hope?
#define FMNADD(a, b, c)		_mm_nmacc_ps(a, b, c)
#endif
...
namespace hdt
{
	...
	static btScalar gResolveSingleConstraintRowGeneric_avx256(btSolverBody& body1, btSolverBody& body2, const btSolverConstraint& c)
	{
		...
		return deltaImpulse[0] / c.m_jacDiagABInv;
	}
	...
	btScalar GroupConstraintSolver::solveGroupCacheFriendlySetup(btCollisionObject** bodies, int numBodies, btPersistentManifold** manifoldPtr, int numManifolds, btTypedConstraint** constraints, int numConstraints, const btContactSolverInfo& infoGlobal, btIDebugDraw* debugDrawer)
	{
		...
		std::for_each(m_groups.begin(), m_groups.end(), [&](ConstraintGroup* i) /* concurrency::parallel_for_each */
		{
		...
		if (m_tmpSolverNonContactConstraintPool.size())
		{
			...
			for (int i = 0; i < m_nonContactConstraintRowPtrs.size(); ++i)
			{
				...
				if (lastA != a || lastB != b)
				{
					if (lastA && lastB)
					{
						auto task = std::static_pointer_cast<SolverTask>(std::make_shared<NonContactSolverTask>(lastA, lastB, lastBegin, curr, getActiveConstraintRowSolverGeneric()));
			...
			if (lastA && lastB)
			{
				auto task = std::static_pointer_cast<SolverTask>(std::make_shared<NonContactSolverTask>(lastA, lastB, lastBegin, m_nonContactConstraintRowPtrs.data() + m_nonContactConstraintRowPtrs.size(), getActiveConstraintRowSolverGeneric()));
				m_tasks.push_back(task);
				m_nonContactTasks.push_back(task);
			}
		}
		...
		for (int j = 0; j < numConstraints; j++)
		{
			if (constraints[j]->isEnabled())
			{
				...
				if (!info1.m_numConstraintRows && !info1.nub)
				{
					...
					auto task = std::static_pointer_cast<SolverTask>(std::make_shared<ObsoleteSolverTask>(bodyA, bodyB, constraints[j], infoGlobal.m_timeStep));
					m_tasks.push_back(task);
					m_nonContactTasks.push_back(task);
				}
			}
		...
		for (int i = 0; i < m_tmpSolverContactConstraintPool.size(); ++i)
		{
			...
			auto task = std::static_pointer_cast<SolverTask>(std::make_shared<ContactSolverTask>(a, b, c, f0, f1, getActiveConstraintRowSolverLowerLimit(), getActiveConstraintRowSolverGeneric()));
			m_tasks.push_back(task);
	...
	btScalar GroupConstraintSolver::solveSingleIteration(int iteration, btCollisionObject** bodies, int numBodies, btPersistentManifold** manifoldPtr, int numManifolds, btTypedConstraint** constraints, int numConstraints, const btContactSolverInfo& infoGlobal, btIDebugDraw* debugDrawer)
	{
		...
		{
			PRINT_FMT("doing caculations.. (%s)", "LOL");
			std::for_each(m_tasks.begin(), m_tasks.end(), [](const SolverTaskPtr& task) { task->solve(); }); /* concurrency::parallel_for_each */
		}
		else
		{
			std::random_device rng;
			...
			PRINT_FMT("doing caculations.. (%s)", "LOL");
			std::for_each(m_nonContactTasks.begin(), m_nonContactTasks.end(), [](const SolverTaskPtr& task) { task->solve(); }); /* concurrency::parallel_for_each */
			std::for_each(m_contactTasks.begin(), m_contactTasks.end(), [](const SolverTaskPtr& task) { task->solve(); }); /* concurrency::parallel_for_each */
		}

...
btMiniSDF::subdomain(btMultiIndex const& ijk) const
{
	...
	btVector3 origin = m_domain.minoo() + tmp;
							...
							btVector3* gradient) const
{
	...
	btVector3 tmpmi = ((x - m_domain.minoo()) * (m_inv_cell_size));  //.cast<unsigned int>().eval();
	...
	btVector3 denom = (sd.maxoo() - sd.minoo());
	btVector3 c0 = btVector3(2.0, 2.0, 2.0) / denom;
	btVector3 c1 = (sd.maxoo() + sd.minoo()) / denom;
